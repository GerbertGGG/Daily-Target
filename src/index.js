...
async function fetchUpcomingRaces(authHeader) {
  ...
  // ✅ Exakt so wie deine API liefert
  ...
  if (DEBUG) {
    ...
    console.log(`🏁 A-Rennen: ${races.length}`);
  }
...
function parseEventDateMs(e) {
  const s = e?.start_date_local || e?.start_date || e?.start_date_local || "";
...
// =========================
// 📈 Trainingslogik (CTL/ATL/ACWR/TSS)
// =========================
...
function simulateFutureWeeks(ctl, atl, weeks = 6) {
  ...
  for (let w = 1; w <= weeks; w++) {
    // vorher war das fix 0.8 – wir lassen es so wie in deinem Code
...
// =========================
// 🧠 Intervall-Empfehlung (nur Text, in Kommentar)
// =========================
function buildIntervalRecommendation({ markers, daysToRace }) {
  const aerobe = markers.find((m) => m.name.includes("Aerobe"));
...
async function handle(dryRun = true) {
  ...
  // 🏁 Rennen abrufen / nächstes Rennen / Sport
  ...
  // 📈 TSS/CTL/ATL Simulation (6 Wochen)
  const progression = simulateFutureWeeks(ctl, atl, 6);

  // ⏱️ Days to race (für Intervall-Empfehlung)
  let daysToRace = null;
  if (nextRace) {
    const t = parseEventDateMs(nextRace);
    if (Number.isFinite(t)) {
      daysToRace = Math.round((t - Date.now()) / (24 * 3600 * 1000));
    }
  }

  // Marker-Auswahl für Intervalle (sportabhängig)
  const intervalMarkers =
    nextRaceSport === "run" ? runTable.markers :
    nextRaceSport === "ride" ? rideTable.markers :
  ...
  const comment = [
    ...
    intervalRec,
    "",
    `**Wochentarget TSS:** ${progression[0]?.weekTss ?? "k.A."}`,
  ...
  if (DEBUG) {
    console.log({
      runDrift, rideDrift, runEff, rideEff,
      phase, progression,
      nextRace: nextRace ? { name: nextRace.name, type: nextRace.type, category: nextRace.category, start: nextRace.start_date_local } : null,
      intervalRec
    });
  }
